...
class ChatDBFireStore {
  ...
  static Future<void> checkUserExists(User logInUser) async {
    final QuerySnapshot result = await FirebaseFirestore.instance.collection(getDocName()).where('userId', isEqualTo: logInUser.uid).get();
  ...
  static Future<void> makeUserOnline(User logInUser) async {
    FirebaseDatabase.instance.reference().child("/status/" + logInUser.uid).onDisconnect().set("offline");

    FirebaseDatabase.instance.reference().child("/status/" + logInUser.uid).set("online");
  }
}
